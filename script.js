...
let attemptCount = 0;
const maxAttempts = 4;
// const blockDuration = 8 * 60 * 60 * 1000; // 8 horas en milisegundos
const blockDuration = 3 * 60 * 1000; // 5 minutos en milisegundos
...
function showCourseDetails(courseId) {
    ...
    if (lastAttempt && Date.now() - lastAttempt < blockDuration) {
...
function openModal() {
    ...
    document.getElementById("modalTitle").textContent = `Acceso: ${currentCourse.name}`;
    document.getElementById("accessCodeInput").value = "";

    setTimeout(() => modal.classList.add("show"), 100);

    document.getElementById("confirmAccess").onclick = checkAccess;
    document.getElementById("cancelAccess").onclick = closeModal;
}
...
function checkAccess() {
    ...
    if (codigo !== currentCourse.accessCode) {
        attemptCount++;
        document.getElementById("accessCodeInput").classList.add("input-error");

        setTimeout(() => {
            document.getElementById("accessCodeInput").classList.remove("input-error");
        }, 600);

        if (attemptCount >= maxAttempts) {
            alert("Actualmente esta bloqueado este usuario.");
